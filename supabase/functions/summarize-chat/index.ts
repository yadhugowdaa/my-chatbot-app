...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: messages, error: messagesError } = await supabaseAdmin
      .from('messages').select('sender, content').eq('chat_id', chat_id).order('created_at', { ascending: true })
    ...
    if (!n8nResponse.ok) throw new Error(`n8n summary workflow failed: ${await n8nResponse.text()}`)
